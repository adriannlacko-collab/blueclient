...
public class HudLayoutScreen extends VanillaScreen {
   ...
   public HudLayoutScreen(class_437 parent) {
      super(parent, class_2561.method_43470("HUD Layout"));
   }
   ...
   protected int content(int top) {
      this.blockWidth = 200;
      this.blockLeft = (this.field_22789 - this.blockWidth) / 2;
      this.method_37063(class_4185.method_46430(class_2561.method_43470("Reset to default").method_27692(class_124.field_1054), button -> {
         Hud.resetLayout();
         this.picked = null;
         this.frozen = false;
         this.method_41843();
      }).method_46434(this.blockLeft, this.field_22790 - 52, this.blockWidth, 20).method_46431());
   ...
   public void method_25394(class_332 ctx, int mouseX, int mouseY, float delta) {
      Hud.stackDefaults(this.field_22787, this.field_22789, this.field_22790);
      super.method_25394(ctx, mouseX, mouseY, delta);
      List<Module> modules = this.placeable();
      Module hovered = this.idle() ? this.at(modules, mouseX, mouseY) : null;
      boolean live = this.field_22787 != null && this.field_22787.field_1687 != null;
      ...
      for (Module module : modules) {
         ...
         if (!live || !module.isVisible(this.field_22787)) {
   ...
   private void frame(class_332 ctx, int x, int y, int w, int h, boolean held) {
      int colour = held ? -1 : -4209460;
      int x0 = x - 2;
      int y0 = y - 2;
      int x1 = x + w + 2;
      int y1 = y + h + 2;
      ctx.method_25294(x0, y0, x1, y0 + 1, colour);
      ctx.method_25294(x0, y1 - 1, x1, y1, colour);
      ctx.method_25294(x0, y0, x0 + 1, y1, colour);
      ctx.method_25294(x1 - 1, y0, x1, y1, colour);
      ctx.method_25294(x1 - 6, y1 - 6, x1, y1, colour);
      ctx.method_25294(x1 - 6 + 2, y1 - 2, x1 - 1, y1 - 1, -14012874);
      ctx.method_25294(x1 - 2, y1 - 6 + 2, x1 - 1, y1 - 1, -14012874);
   }

   private void ghost(class_332 ctx, Module module, int x, int y, int w, int h) {
      ctx.method_25294(x, y, x + w, y + h, -1873784752);
      ctx.method_51433(this.field_22793, module.name, x + 1, y + 1, -2039584, false);
   }

   private void guides(class_332 ctx) {
      if (this.guideX >= 0) {
         ctx.method_25294(this.guideX + 1, 1, this.guideX + 2, this.field_22790, -12632257);
         ctx.method_25294(this.guideX, 0, this.guideX + 1, this.field_22790, -1);
      }

      if (this.guideY >= 0) {
         ctx.method_25294(1, this.guideY + 1, this.field_22789, this.guideY + 2, -12632257);
         ctx.method_25294(0, this.guideY, this.field_22789, this.guideY + 1, -1);
      }
   }

   private void readout(class_332 ctx) {
      if (this.resizing != null && this.resizing.scale() != null) {
         class_2561 text = Fonts.ui(Math.round(this.resizing.scale().get() * 100.0F) + "%");
         ...
         int x = Math.max(2, Math.min(this.field_22789 - this.field_22793.method_27525(text) - 2, cx - this.field_22793.method_27525(text) / 2));
         ctx.method_27535(this.field_22793, text, x, Math.min(this.field_22790 - 10, y), -1);
      }
   }
   ...
   private int boxWidth(Module module) {
      return module.isVisible(this.field_22787) ? module.scaledWidth(this.field_22787) : this.field_22793.method_1727(module.name) + 2;
   }

   private int boxHeight(Module module) {
      return module.isVisible(this.field_22787) ? module.scaledHeight(this.field_22787) : 9;
   }
   ...
   private int placedX(Module module, int w) {
      ...
      if (stacked != null) {
         ...
         return home != null ? home[0] : module.position().screenX(this.field_22789, w);
      }
   }
   ...
   private int placedY(Module module, int h) {
      ...
      if (stacked != null) {
         ...
         int[] home = this.vanillaHome(module);
         return home != null ? home[1] : module.position().screenY(this.field_22790, h);
      }
   }

   private int[] vanillaHome(Module module) {
      return module.position().isDefault() ? module.vanillaPlace(this.field_22787) : null;
   }
   ...
   private void place(Module module, int x, int y, int w, int h) {
      module.position()
         .moveTo(this.field_22789 - w <= 0 ? 0.0F : (float)x / (this.field_22789 - w), this.field_22790 - h <= 0 ? 0.0F : (float)y / (this.field_22790 - h));
   }
   ...
   protected boolean onDrag(double mouseX, double mouseY, int button, double dx, double dy) {
      ...
      if (this.resizing != null) {
         ...
         if (this.guideX >= 0 && this.guideX != this.heldX || this.guideY >= 0 && this.guideY != this.heldY) {
            Sounds.ui(this.field_22787, (class_3414)class_3417.field_15015.comp_349(), 1.7F, 0.25F);
         }
   ...
   private int snap(int value, int size, boolean horizontal) {
      int screen = horizontal ? this.field_22789 : this.field_22790;
