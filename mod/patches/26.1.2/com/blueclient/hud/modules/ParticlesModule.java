...
public class ParticlesModule extends BehaviourModule {
   ...
   // Never 0: the mixin returns null from createParticle for 0, and the game's own callers use what it
   // returns (a firework's Starter calls setTrail on each spark, so a skipped spark crashed the game).
   // A particle to skip is made and handed back as usual, and keeps() stops it at ParticleEngine.add.
   public static int copies(ParticleOptions effect) {
      if (governs() && !copying) {
         int kind = kind(effect);
         int copies;
         if (kind == 3) {
            copies = !emitting && effect != ParticleTypes.EXPLOSION_EMITTER ? Math.min(1, share(1, active.explosion)) : 1;
         } else {
            copies = kind != 0 ? 1 : share(1, active.others);
         }
   ...
   public static boolean keeps() {
      if (naming > 0 && naming < 64 && (skipped & 1L << naming) != 0L) {
         return false;
      } else {
   ...
   // The mixin calls naming(true) right after copies() returned 1 and naming(false) in a finally around
   // the game's createParticle, so a skip is kept per depth: a particle made inside another's (a provider
   // that makes particles) has its own bit, and the bit is gone when its createParticle returns.
