...
public final class Hud {
   ...
   private static Module besideArmour;
   ...
   public static void register() {
      ...
      MODULES.add(
         new SmallItemsModule(
            "smallblocks", "Small blocks", "Any block, smaller in your hand", () -> Items.COBBLESTONE, stack -> stack.getItem() instanceof BlockItem
         )
      );
      MODULES.add(
         new SmallItemsModule(
   ...
   public static List<Module> onScreen(Minecraft client) {
      VISIBLE.clear();
   ...
   private static void applyingNotice(GuiGraphicsExtractor ctx, Minecraft client) {
      String text = "Applying shaders...";
      int w = client.font.width(text);
      int x = (ctx.guiWidth() - w) / 2;
      int y = 6;
      ctx.fill(x - 2, y - 1, x + w + 2, y + 9, -1873784752);
      ctx.text(client.font, text, x, y, -2039584, false);
   ...
   public static void renderCrosshair(GuiGraphicsExtractor ctx, Minecraft client) {
      Module module = crosshair;
      if (module != null && module.isEnabled() && client.player != null) {
         module.render(ctx, client, x(module, client, ctx.guiWidth()), y(module, client, ctx.guiHeight()));
      }
   }

   public static int crosshairIndicatorDrop(GuiGraphicsExtractor ctx, Minecraft client) {
      Module module = crosshair;
   ...
   public static void stackDefaults(Minecraft client, int screenW, int screenH) {
      stackingFrame++;
      STACKABLE.clear();
      boxCount = 0;
      Arrays.fill(USED, 0);
      ...
      if (!paired && placedArmour != null && totems != null) {
         int aw = placedArmour.scaledWidth(client);
         int ah = placedArmour.scaledHeight(client);
         int ax = placedArmour.position().screenX(screenW, aw);
         int ay = placedArmour.position().screenY(screenH, ah);
         int tw = totems.scaledWidth(client);
         int th = totems.scaledHeight(client);
         int tx = ax - 2 - tw;
         if (tx < 0) {
            tx = ax + aw + 2;
         }

         int ty = Math.max(0, Math.min(screenH - th, ay + ah - th));
         keepOut(tx, ty, tw, th);
         stack(totems, tx, ty);
         besideArmour = totems;
      }
      ...
      for (int ix = 0; ix < STACKABLE.size(); ix++) {
         ...
         if (module != besideArmour && (!paired || module != totems) && (!clicksBeside || module != cps)) {
            Anchor anchor = module.anchor();
            int w = module.scaledWidth(client);
            int h = module.scaledHeight(client);
   ...
   public static boolean followsArmour(Module module) {
      return module != null && module == besideArmour && stackedOf(module) != null;
   }

   public static void draw(GuiGraphicsExtractor ctx, Module module, Minecraft client, int screenW, int screenH) {
   ...
   public static int x(Module module, Minecraft client, int screenW) {
   ...
   public static int y(Module module, Minecraft client, int screenH) {
      int[] stacked = stackedOf(module);
      if (stacked != null) {
         return stacked[1];
      } else {
