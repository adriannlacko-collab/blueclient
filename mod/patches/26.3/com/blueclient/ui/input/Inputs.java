...
public final class Inputs {
   ...
   /**
    * Same answer as InputConstants.isKeyDown(key): SDL's keyboard state array,
    * which SDL documents as valid for the whole life of the application. The
    * game's method wraps it in a new ByteBuffer on every call (several per
    * frame for the HUD's key checks); this keeps the first wrapper.
    */
   public static boolean keyDown(int key) {
      ByteBuffer state = keys;
      if (state == null) {
         state = SDLKeyboard.SDL_GetKeyboardState();
         if (state == null) {
            return false;
         }
